#!/usr/bin/env bash
#
# build-linux.sh — Build and run Wagic the Homebrew on Linux (Ubuntu/Debian)
#
# Usage:
#   ./scripts/build-linux.sh          # build + run
...
clone_source() {
    if [ -d "$WAGIC_DIR/.git" ]; then
        info "Wagic source already cloned at $WAGIC_DIR. Pulling latest..."
        cd "$WAGIC_DIR"
...
build_wagic() {
    ...
    if command -v qmake-qt5 &>/dev/null; then
        ...
        err "qmake not found. Install Qt5 dev packages first (run: $0 deps)"
        exit 1
    fi
    ...
    make -j"$(nproc)"
    ok "Build complete: $BUILD_DIR/wagic"
}
